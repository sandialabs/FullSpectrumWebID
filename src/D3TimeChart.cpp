  ...
  m_chartBackgroundColor( 0x00, 0x00, 0x00 )
{
  ...
  wApp->require( "D3TimeChart.min.js" );
  wApp->useStyleSheet( "D3TimeChart.min.css" );
#else
  wApp->require( "D3TimeChart.js" );
  wApp->useStyleSheet( "D3TimeChart.css" );
#endif
...
void D3TimeChart::defineJavaScript()
{
  ...
  // For FullSpectrum we dont care about receiving client side events, so we'll skip it; same for
  //  selecting foreground/background/secondary - but note these options have not been "upstreamed"
  //  to the InterSpec code base.
  options += ", noEventsToServer:" + jsbool(true);
  options += ", noTimeRegionSelect:" + jsbool(true);
  options += "}";
...
void D3TimeChart::setDataToClient()
{
  ...
  // A hack for FullSpectrum.  We'll make sure background doesnt take up too much space.
  double backgroundTime = 0.0, totalTime = 0.0;
  for( const int sample_num : sample_numbers )
  {
    ...
    totalTime += realTime;
    if( sourcetype == SpecUtils::SourceType::Background )
      backgroundTime += realTime;
  }//for( const int sample_num : sample_numbers )
...
#define Q_DBL_NaN std::numeric_limits<double>::quiet_NaN()
  ...
  for( const int sample_num : sample_numbers )
  {
    ...
    // FullSpectrum specific hack
    // Make sure the background wont take up more than 20% of the timeline.
    if( (sourcetype == SpecUtils::SourceType::Background)
       && (totalTime > 1.0) && (backgroundTime > 1.0) && ((totalTime - backgroundTime) > 0.5)
       && (backgroundTime > 0.2*totalTime) )
    {
      const double scale = 0.2*(totalTime - backgroundTime) / backgroundTime;
      realTime *= scale;
    }
  ...
  if( anyStartTimeKnown )
  {
    ...
    js << ",\n\t\"startTimeOffset\": " << static_cast<long long int>(startTimesOffset);
    js << ",\n\t\"startTimes\": ";
    for( size_t i = 0; i < startTimes.size(); ++i )
    {
      ...
      if( startTimes[i] == std::numeric_limits<int64_t>::min() )
        ...
        js << static_cast<long long int>(startTimes[i]);
    }
    js << "]";
  }//if( anyStartTimeKnown )
                                           ...
                                           const SpecUtils::SpectrumType type )
{
  ...
  switch( type )
  {
    case SpecUtils::SpectrumType::Foreground:
      region.color = Wt::WColor(255,255,0,155);
      break;
    case SpecUtils::SpectrumType::SecondForeground:
      region.color = Wt::WColor(0,255,255,75);
      break;
    case SpecUtils::SpectrumType::Background:
      region.color = Wt::WColor(0,128,0,75);
      break;
  }//switch( type )
...
void D3TimeChart::render( Wt::WFlags<Wt::RenderFlag> flags )
{
  ...
  //const bool renderUpdate = (flags & Wt::RenderFlag::Update);
  ...
  m_renderFlags = Wt::WFlags<TimeRenderActions>{};
